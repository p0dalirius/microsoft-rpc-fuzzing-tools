#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# File name          : RpcInterfaceVersionFuzzer.py
...
class RpcInterfaceVersionFuzzer(object):
    uuid = None
    version = None
    pipe = None
    ...
    def __init__(self):
        super(RpcInterfaceVersionFuzzer, self).__init__()
...
if __name__ == '__main__':
    ...
    fuzzer = RpcInterfaceVersionFuzzer()

    fuzzer.uuid = options.uuid
    fuzzer.pipe = options.pipe

    for version in track(range(0,100)):
        version = version/10
        fuzzer.connect(
        ...
        connected = fuzzer.bind(uuid=options.uuid, version=str(version))
        if connected:
            print("\x1b[1;92m[+] Interface (uuid=%s, version=%s) exists.\x1b[0m" % (options.uuid, version))
